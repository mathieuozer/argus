    ...
    <div>
      <div className="page-header">
        <div className="page-header-left">
          <h2>SLO Dashboard</h2>
          <p>Service Level Objectives and error budget tracking</p>
        </div>
        <div className="page-header-actions">
          <button className="btn btn-primary" onClick={() => setShowEditor(true)}>Create SLO</button>
          <button className="btn" onClick={() => fetchSLOs()} disabled={loading}>Refresh</button>
        </div>
      </div>
      ...
      {slos.length > 0 && (
        <div className="stat-row">
          <div className="stat-card">
            <span className="stat-label">Total SLOs</span>
      ...
      {!loading && slos.length === 0 && !showEditor && (
        <div className="empty-state">
          ...
          <p>Create SLOs to track availability, latency, and error rate targets for your agents.</p>
          <button className="btn btn-primary" onClick={() => setShowEditor(true)}>Create First SLO</button>
        </div>
      )}
    </div>
  );
